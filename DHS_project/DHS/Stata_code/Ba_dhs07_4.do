...
* 3 categories(job): 
  * Physical labor(12, 13, 14, 15,16) ;
  * Service or business(11, 21, 22, 23, 31, 41, 51, 52) ;
  * Others (0, 61, 96)
  
gen job = 1 if  inlist(occup, 12, 13, 14, 15, 16)
replace job = 2 if inlist(occup, 11, 21, 22, 23, 31, 41, 51, 52)
replace job = 3 if inlist(occup, 0, 61, 96)

* Define labels 

label define job 1 "Physical labor" 2 "Service or business"  3"others/unemployed"
 ...
 * 1.2 (hv202) Main source of Non-drinking water
  ...
  * <(use_water)Recoding Non-drinking water criteria>
 * 2 categories: Improved drinking water      vs. Unimproved drinking water 
 *===============================================================================
...
foreach var in male agegroup marit religion region edu_level edu_atm cw pastwork ayemp tearn job{
    tab `var' new_ru, col chi2
	tab `var' ruyr, col chi2
...
*SES restricted to men.
foreach var in edu_level edu_atm cw pastwork ayemp tearn {
    tab `var' new_ru if male==1, col chi2
	tab `var' ruyr if male==1,col chi2
...
foreach var of varlist male agegroup marit religion region edu_level edu_atm cw pastwork ayemp tearn {
    tab `var', m
}

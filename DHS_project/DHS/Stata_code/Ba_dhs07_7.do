...
summarize nhhid
label variable nhhid "household ID"

encode caseid, gen (ncaseid)
summarize ncaseid
label variable ncaseid "individual case id"

* correction of labels for gender 
rename caseid fcaseid 
...
*** 3.3 Merge cluster(361 psus): with community level data for health facility 

gen cluster =  v001 if male == 0 
replace cluster =  mv001 if male == 1 

drop _merge 
save "$path/fullsample.dta", replace

clear 
use "$path/BDSQ51DT/bdCQ51FL.dta",clear
rename coclust cluster 
merge 1:m cluster using "$path/fullsample.dta"
drop _merge 

* inspection 
compare hhid nhhid

*  matched: 14,767  (_merge==3)
save "$path/fullsample.dta", replace

* There are 361 clusters, 9,289 households and 14,767 individuals in the dataset before further cleaning;

*****************************************************
/* II Variables Creation     */ 
*****************************************************

* <mv101 + v101> Division (6)
gen division = v101 if male == 0 
replace division = mv101 if male == 1 
tab division, m 
label variable division "6 divisions"
...
sum division nhhid ncaseid cluster

* There are 361 psus, 9,289 households and 13,690 individuals after dropping visitors.
...
replace job = 6 if inlist(occup,  41, 51, 52) //professional & managerial
replace job = 7 if inlist(occup, 96)
...
* 9.2 original continuous BMI (Body mass Index) restrcited to women Not pregnant

replace v445 = . if v445 == 9998 | v445 == 9999
gen bmi = v445/100 if v213 != 1
summarize bmi, detail

* 9.3 BMI Groups(Body mass Index) restrcited to women Not pregnant
...
******************* Community data: Identification of Health Facilities

foreach var of varlist co701a co701bn co701bu{
 replace `var' = .  if `var'== 9 | `var' == 98 
 }
 
** Health Facility 
* 1. <pharm> co510:any pharmacies in the village
gen pharm = 1 if co510 == 1
replace pharm = 0 if co510 == 2
label variable pharm "pharmacy in the village"
label define pharm 0 "no" 1 "yes"
label value pharm pharm 

* 2. <distance> standardized distance for measurement to the nearest pharmacy

* (co512u) Distance to the nearest pharmacy (unit) 1= mile, 2 = kilometer --> mile to kilometer (2 decimals) 
* (co512n) Distance to the nearest pharmacy
gen pharmdist = round(1.60934*co512n, 0.01) if co512u == 1
replace pharmdist = co512n if co512u == 2
tab pharmdist, m 

* 3. <sba>  co701a: working in the village
 gen sba = 1 if co701a == 1
 replace sba = 0 if co701a == 2
 label variable sba "sba fieldworker"
 label define sba 0 "no" 1 "yes"
 label value sba sba

* 4. <sba distance>  Distance to the nearest sba
gen sbadist = round(1.60934*co701bn, 0.01) if co701bn == 1
replace sbadist = co701bn if  co701bn == 2
tab sbadist, m 
 
* 5. <cnumrec2> number of health facilities
 sum cnumrec2, detail
 
* 6. <cnumrec3> number of all family planning workers and healthcare workers
 sum cnumrec3, detail
 
********* Generate Binary variables for health resources based on (cnumrec3- cnumrec6)****
* 1. <facility> number of health facility (cnumrec2)
gen facility = 0 if cnumrec2 == 0 
replace facility = 1 if cnumrec2 > 0

* 2. <hworker> number of health and fp workers (cnumrec3)
gen hworker = 0 if cnumrec3 == 0 
replace hworker = 1 if cnumrec3 > 0 


* 3. <depot> number of depot holders (cnumrec4)
gen depot = 0 if cnumrec4 == 0 
replace depot = 1 if cnumrec4 > 0 

* 4. <doctor> number of doctors (cnumrec6)
gen doctor =  0 if cnumrec6 == 0 
replace doctor = 1 if cnumrec6 > 0 

*** (cworker, cdepot, cdoc) Generate mean value of health service providers by cluster(PSUs) 
egen cfacility = mean(cnumrec2), by(cluster)
egen cworker = mean(cnumrec3), by(cluster)
egen  cdepot= mean(cnumrec4),by(cluster)
egen cdoc = mean(cnumrec6),by(cluster)
...
***Health Facility 
foreach var of varlist pharm sba  {
    tab `var' new_ru, col chi2
	tab `var' ruyr,col chi2
}
...
*SES restricted to women
foreach var in edu_level edu_atm cw pastwork ayemp tearn sm121{
	tab `var' ruyr if male==0, col chi2
...
***Health Measurements
 foreach var of varlist cfacility  cworker cdoc {
  ttest `var', by(mg) 
  ttest `var', by(rumg)
  ttest `var', by(new_ru)
}
...
* anova for BMI
oneway bmigroup mg, bonferroni tabulate
oneway bmigroup rumg, bonferroni tabulate

****** Weighting the data (Not included in Analysis) 
* Potential variables for using: 
* mv005, v005, 

** Step 1: create the weight variable (always divide by 1,000,000)
gen wgt = v005/1000000
replace wgt = . if male == 1

gen mgt = mv005/1000000
replace mgt = . if male == 0


** Step 2: weight the data and apply the sample weight 
foreach var of varlist hv201 hv225 hv206 hv205 adv {
    tab `var'[iweight = wgt]
...
foreach var of varlist hv201 hv225 hv206 hv205 adv {
    tab `var'[iweight = mgt]
	svyset [pw = mgt], psu(mv021) strata(mv022)
	svy: tab `var'
}


** No significant change for distribution after adjusting the weight, for further decision. 

**************** Visualization 

** Health facilities(cfacility)
** Mean No. of health facilities per cluster/PSU
graph hbar (mean) cfacility, over(mg) over(wealth) title(Mean No. of facilities in each PSU differences)  

** (cdoc) No. of doctors per PSU Inequality : 
 graph hbar (mean) cdoc, over(new_ru)  over(wealth) title(Mean No. of doctors in each PSU differences,  span pos(11))  subtitle(migrants and non migrants) 

** Health worker by edu 
vioplot cworker, over(new_ru) title(Mean No. of health workers differences) subtitle(urban non migrants vs urban-rural migrants)
graph bar (mean) cworker, over(new_ru) by(edu_atm)

** Pharmacy disttance by Income (Add Wealth Index)
grmeanby wealth edu_level rumg new_ru mg , summarize(pharmdist) ytitle(kilometers) ytitle(, size(medlarge)) title(Mean distance differences) subtitle(community to pharmacies)


******Note
* 1. The PSU(primary sampling unit) is usually the same as the cluster;
* 2. Strata varies by country;
* 3. The weight variable is only applicable to certain groups of people. 
* 4. Cluster level weight/thana weight and union level weight all missing;
* 5. SBA = skilled birth attendance
* 6. fp = family practice 
...
* 3. Gazi, R., Mercer, A., Khatun, J., & Islam, Z. (2005). Effectiveness of depot-holders introduced in urban areas: Evidence from a pilot in Bangladesh. Journal of Health, Population and Nutrition, 377-387.
